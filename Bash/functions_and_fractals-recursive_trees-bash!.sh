...
draw_fractal() {
    ...
    # If depth equals 0, end the recursion
    [[ $depth -eq 0 ]] && return

    # Draw the trunk
    for ((i=length; i; i--)); do
        fractal_array[$((row-i)).$column]=1
    done

    # Move the row up to the start of the branches
    ((row -= length))

    # Draw the branches
    for ((i=length; i; i--)); do
        fractal_array[$((row-i)).$((column-i))]=1
        fractal_array[$((row-i)).$((column+i))]=1
    done

    # Recurse for the next depth level
    draw_fractal $((depth-1)) $((length/2)) $((row-length)) $((column-length))
    draw_fractal $((depth-1)) $((length/2)) $((row-length)) $((column+length))
}

read depth_level

# Initiate the fractal drawing process
draw_fractal $depth_level 16 63 49

# Output the fractal
for ((i=0; i<63; i++)); do
    for ((j=0; j<100; j++)); do
        if [[ ${fractal_array[$i.$j]} ]]; then
            printf 1
        else
            printf _
        fi
